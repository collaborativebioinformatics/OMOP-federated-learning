...
def heterogeneity(axis: plt.Axes, results: dict) -> None:
    ...
    axis.set_ylabel("training people", color=MUTED, fontsize=12)
    axis.set_xticks(range(len(names)))
    axis.set_xticklabels([name.replace("centre_", "") for name in names], rotation=45, ha="right", fontsize=10)
    twin = axis.twinx()
    twin.plot(range(len(names)), incidence, "o-", color=FEDERATED, markersize=4, linewidth=1.2)
    twin.set_ylabel("incident cases (%)", color=FEDERATED, fontsize=12)
    twin.set_ylim(bottom=0)
    axis.set_title("What each site holds", fontsize=13)
...
def generalisation(axis: plt.Axes, results: dict) -> None:
    ...
    axis.set_xticklabels(
        [name.replace("centre_", "") for name in names] + ["fed", "pool"], rotation=45, ha="right", fontsize=10
    )
    axis.set_ylabel("AUROC on the pooled held-out cohort", fontsize=12)
    axis.set_ylim(0.0, 1.0)
    axis.legend(fontsize=10, frameon=False, loc="upper left")
    axis.set_xlim(-0.8, len(order) - 0.2)
    spread = np.nanstd(local)
    axis.set_title(f"Generalisation (local spread {spread:.3f})", fontsize=13)
...
def coefficients(axis: plt.Axes, results: dict) -> None:
    ...
    axis.set_xticklabels(features, rotation=45, ha="right", fontsize=10)
    axis.set_ylabel("coefficient", fontsize=12)
    axis.legend(fontsize=10, frameon=False)
    axis.set_title("Single-site estimates scatter", fontsize=13)
